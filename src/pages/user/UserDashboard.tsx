      ...
      <div className="container mx-auto px-6 -mt-40 relative z-10">
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          ...
          <div className="lg:w-1/3 space-y-8">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-white">
              <div className="flex flex-col items-center text-center">
                ...
                <h2 className="text-xl md:text-2xl font-bold text-slate-900 tracking-tight mb-1">{user.name}</h2>
                ...
                <div className="w-full space-y-2">
                  ...
                  <NavItem icon={<History size={18} />} label="Riwayat Peminjaman" active={activeTab === "loans"} onClick={() => setActiveTab("loans")} />
                  <NavItem icon={<Settings size={18} />} label="Pengaturan Profil" active={activeTab === "settings"} onClick={() => setActiveTab("settings")} />
            ...
            <AnimatePresence mode="wait">
              {activeTab === "card" && (
                <motion.div key="card" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                  <div className="bg-white p-6 md:p-10 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl shadow-slate-200 border border-white">
                    ...
                    <div className="w-full max-w-lg mx-auto overflow-hidden rounded-[1.5rem] md:rounded-[2rem] shadow-2xl shadow-primary/30 group">
                      <div className={`aspect-[1.6/1] w-full bg-gradient-to-br ${levelClass} p-5 md:p-8 text-white relative flex flex-col justify-between`}>
                        <div className="absolute top-0 right-0 w-32 h-32 md:w-48 md:h-48 bg-white/10 rounded-full -mr-16 -mt-16 blur-3xl transition-all group-hover:scale-110" />
                        ...
                        <div className="relative z-10 flex justify-between items-start">
                          <div className="flex items-center gap-2 md:gap-3">
                            ...
                            <div className="text-[8px] md:text-[10px] font-black uppercase tracking-widest leading-none">
                              Cingkrong<br/><span className="text-white/60">Digital Library</span>
                            </div>
                          </div>
                          <div className="text-[7px] md:text-[8px] font-black bg-white/20 px-2 md:px-3 py-1 md:py-1.5 rounded-full tracking-widest">PREMIUM ACCESS</div>
                        </div>
                        
                        <div className="relative z-10">
                          <p className="text-[7px] md:text-[8px] font-bold text-white/60 uppercase tracking-widest mb-0.5 md:mb-1">Nama Lengkap Anggota</p>
                          <h2 className="text-lg md:text-2xl font-bold tracking-tight mb-3 md:mb-4 truncate">{user.name}</h2>
                          <div className="flex justify-between items-end border-t border-white/20 pt-3 md:pt-4">
                            <div>
                              <p className="text-[7px] md:text-[8px] font-bold text-white/60 uppercase tracking-widest">Tingkat Pembaca</p>
                              <p className="text-[10px] md:text-xs font-black uppercase tracking-widest text-white">{readerLevel}</p>
                            </div>
                            <div className="text-right">
                              <p className="text-[7px] md:text-[8px] font-bold text-white/60 uppercase tracking-widest">Nomor Identitas</p>
                              <p className="font-mono text-[10px] md:text-xs">{user.id.toString().padStart(6, '0')}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="mt-8 md:mt-12 flex justify-center">
                      <button onClick={() => window.print()} className="w-full md:w-auto flex items-center justify-center gap-3 px-8 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all shadow-xl shadow-slate-200">
                  ...
                  <div className="grid grid-cols-3 gap-3 md:gap-6">
                    <StatBox label="Pinjaman Aktif" value={activeLoans.length} color="text-primary" />
                    <StatBox label="Buku Selesai" value={completedLoansCount} color="text-emerald-500" />
                    <StatBox label="Poin Literasi" value={totalPoints} color="text-orange-500" />
                  </div>
                </motion.div>
              )}
                ...
                <motion.div key="loans" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                  <div className="bg-white p-6 md:p-10 rounded-[2.5rem] md:rounded-[3rem] shadow-xl shadow-slate-100 border border-slate-50">
                    <div className="flex items-center justify-between mb-8 md:mb-10">
                      <h3 className="text-xs font-black text-slate-900 uppercase tracking-[0.2em]">Peminjaman Buku Aktif</h3>
                    ...
                    {activeLoans.length > 0 ? (
                      <div className="space-y-4">
                        {activeLoans.map((loan) => {
                          ...
                          return (
                            <div key={loan.id} className="flex flex-col sm:flex-row items-start sm:items-center p-4 rounded-3xl bg-slate-50/50 border border-slate-50 group hover:bg-white hover:shadow-xl hover:shadow-slate-100 transition-all gap-4">
                              ...
                              <div className="flex-grow">
                                ...
                                <div className="flex gap-4">
                                  <div className="flex items-center text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
                                    <Clock size={12} className="mr-1" /> Tanggal Jatuh Tempo: {loan.dueDate}
                                  </div>
                                </div>
                              </div>
                              <span className={`w-full sm:w-auto text-center text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-xl ${loan.status === 'overdue' ? 'bg-accent/10 text-accent' : 'bg-primary/10 text-primary'}`}>
                                {loan.status === 'overdue' ? 'Terlambat Dikembalikan' : 'Sedang Dipinjam'}
                              </span>
                            </div>
                          );
                        })}
                      ...
                      <div className="text-center py-20 bg-slate-50/50 rounded-[2.5rem] border border-dashed border-slate-200">
                        ...
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">Belum ada peminjaman buku aktif</p>
                      </div>
                    )}
                  </div>
                </motion.div>
              ...
              {activeTab === "settings" && (
                <motion.div key="settings" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  <div className="bg-white p-6 md:p-16 rounded-[2.5rem] md:rounded-[3rem] shadow-xl shadow-slate-100 border border-slate-50">
                    <h3 className="text-xs font-black text-slate-900 uppercase tracking-[0.2em] mb-12">Pengaturan Informasi Akun</h3>
                    <form onSubmit={handleUpdateProfile} className="max-w-md">
                      <div className="space-y-2">
                        <label className={labelClass}>Nama Lengkap Pengguna</label>
                        <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className={inputClass} required />
                      </div>
                      <div className="space-y-2">
                        <label className={labelClass}>Alamat Email Terdaftar</label>
                        <input type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className={inputClass} required />
                      </div>
                      <div className="pt-8">
                        <button type="submit" disabled={isSaving} className="w-full md:w-auto px-10 py-4 bg-slate-900 hover:bg-slate-800 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-lg transition-all active:scale-95 disabled:opacity-50">
                          {isSaving ? "Sedang Menyimpan..." : "Simpan Perubahan Profil"}
                        </button>
                      </div>
                    </form>
                  </div>
...
function NavItem({ icon, label, active, onClick }: { icon: any, label: string, active: boolean, onClick: () => void }) {
  return (
    <button onClick={onClick} className={`flex items-center justify-between w-full px-4 md:px-6 py-4 rounded-2xl transition-all ${active ? "bg-slate-900 text-white shadow-xl shadow-slate-200" : "text-slate-400 hover:bg-slate-50"}`}>
      <div className="flex items-center gap-4">
        {icon}
        <span className="text-[10px] md:text-[11px] font-black uppercase tracking-widest text-left">{label}</span>
      </div>
      <ChevronRight size={14} className={active ? "opacity-100" : "opacity-0 invisible md:visible"} />
    </button>
  );
}
...
function StatBox({ label, value, color }: { label: string, value: number, color: string }) {
  return (
    <div className="bg-white p-4 md:p-6 rounded-2xl md:rounded-3xl border border-slate-100 text-center shadow-sm">
      <p className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 leading-tight">{label}</p>
